...
3. **Save unique prompt file** — Writes `data/prompts/<timestamp>-<slug>.txt`.
4. **Call Replicate API** — One call per image slot to the image model in `contracts/MODELS.json` (hard cap applies; no retries).
5. **Save print-ready PNGs** — Resized/cropped with `sharp` to `data/output/<runId>-poster-XX.png`.
6. **Update poster log** — Appends a row to [`POSTERS.md`](POSTERS.md).

```mermaid
flowchart TD
  nlp[NLP prompt] --> structure[Structure fields]
  structure --> savePrompt[Save data/prompts/unique.txt]
  savePrompt --> api[Replicate API x N]
  api --> png[Save data/output PNGs]
...
Research-produced prompts (same generator):
